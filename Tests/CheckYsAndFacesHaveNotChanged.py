...
def CheckYsAndFacesHaveNotChanged(Geo = None,newTets = None,Geo_new = None): 
    ...
    for cellId in np.array([aliveCells,debrisCells]).reshape(-1):
        ...
        assert_(Geo.Cells(cellId).Y(np.logical_and(tetsToCheck,np.any(ismember(Geo.Cells(cellId).T,Geo.XgID),2)),:)==Geo_new.Cells(cellId).Y(np.logical_and(tetsToCheck_new,np.any(ismember(Geo_new.Cells(cellId).T,Geo.XgID),2)),:))
