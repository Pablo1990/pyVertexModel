...
if not os.path.exists(os.path.join(folder, 'all_simulations_metrics.xlsx')):
    ...
    all_ARs = []
    all_model_names = []
    for _, full_path in enumerate(lst):
            ...
            if os.path.exists(os.path.join(full_path, 'before_ablation.pkl')):
                ...
                if recoiling_info is not None:
                    ...
                    if len(recoiling_info[0]['edge_length_final_normalized']) > 1:
                        edge_length_normalised = np.mean([recoiling['edge_length_final_normalized'][-1] / recoiling['time_steps'][-1] for recoiling in recoiling_info])
                    else:
                        edge_length_normalised = np.mean([recoiling['edge_length_final_normalized'] / recoiling['time_steps']  for recoiling in recoiling_info])
                    ...
                    all_ARs.append(file_splitted[3])
                    all_model_names.append(file_splitted[2])

    # Save the Ks and initial recoils into a csv file
    df_metrics = pd.DataFrame({'K': all_Ks, 'initial_recoil': all_initial_recoils, 'final_edge_normalised': all_final_edges_normalised,
